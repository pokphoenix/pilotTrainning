angular.module('remmath.controllers', ['ngAnimate'])
    .controller('RemMathCtrl', function ($rootScope,$scope) {
        // Debug
        $scope.currentPlatformVersion = ionic.Platform.version();
    })
    .controller('RemMathMainCtrl', function ($rootScope, $scope, $localStorage, $filter, $cordovaAppRate, $location, SemiSound, SemiGC) {
        if($rootScope.deviceReady){
            SemiSound.play('opengame');
        }
        $scope.modeSelect = function(mode) {
            $rootScope.deviceReady = false;
            $rootScope.semi.mode = mode;
            $location.path('geomath/play');
        };
    ...
    .controller('RemMathFooterCtrl', function ($scope,$location) {
        $scope.credit = function () {
            $location.path('remmath/credit');
        };
    })
    .controller('RemMathShowMathScoreCtrl', function ($rootScope, $scope,$location, $localStorage, Semi,$cordovaScreenshot,SemiSound,$cordovaSocialSharing, SemiGC, $timeout) {
        ...
        $timeout(function () {
            $location.path('/remmath/play');
        }, 3000);


    })

    .controller('RemMathGameOverCtrl', function ($rootScope, $scope,$location, $localStorage, Semi,$cordovaScreenshot,SemiSound,$cordovaSocialSharing, SemiGC, $timeout) {
